...
class Argparse
{
    ...
    public Dictionary<string, string> Parse(string[] args)
    {
        ...
        for (int i = 0; i < args.Length; i++)
        {
            ...
            {
                // arg_name = arg;
                if (arguments.Find(a => a.Name == arg_name && a.IsFlag) != null)
                    arg_val = "true";

                else if (arguments.Find(a => a.Name == arg_name && !a.IsFlag) != null)
                {
                    List<string> list = args.ToList();
                    int index = list.IndexOf(arg_name);
                    if (index != -1 && index < args.Length-1)
                        arg_val = args[index + 1];

                    else
                        new Error($"Missing value for argument: {arg_name}");
                }
