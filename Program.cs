    ...
    class Lexer
    {
        ...
        private void Tokenizer()
        {
            ...
            for (int i = 0; i < NonStringStr.Length; i++)
            {
                ...
                if (i == NonStringStr.Length - 1 && !Collections.String.IsEmpty(Lexemes))
                {
                    Toks.Add(Lexemes);
                    Lexemes = "";
                    Expression = "";
    ...
    class Terminal
    {
        ...
        public void Credits()
        {
            string[] CreditCenter = {
                ...
                "|| and may lead system instability.",
                "",
    ...
    class Program
    {
        ...
        static public void AOs(string input="", bool RunIndependent=true, bool RanOnBoot=true)
        {
            ...
            while (Loop)
            {
                ...
                {
                    string Prompt = "";
                    if (Args.Length == 0 || Args == null) AOs.Prompt = "$ ";
                    else
                    {
                        foreach (string i in Args)
                        {
                            ...
                            else if (i.ToLower() == "-v" || i.ToLower() == "--ver" || i.ToLower() == "--version") Prompt += AOs.Version;
                            else if (i.ToLower() == "-s" || i.ToLower() == "--space") Prompt += " ";
                            else if (i.ToLower() == "-t" || i.ToLower() == "--time") Prompt += DateTime.Now.ToString("HH:mm:ss");
                            else if (i.ToLower() == "-d" || i.ToLower() == "--date") Prompt += DateTime.Now.ToString("dd-MM-yyyy");
                            else if (i.ToLower() == "-p" || i.ToLower() == "--path") Prompt += Directory.GetCurrentDirectory();
                            else if (i.ToLower() == "-n" || i.ToLower() == "--drive") Prompt += Path.GetPathRoot(Environment.SystemDirectory);
